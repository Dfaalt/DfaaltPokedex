...
// Values: 2 = super effective, 0.5 = not very effective, 0 = no effect, 1 = neutral
...
export function calculateTypeEffectiveness(types: string[]): {
  ...
  types.forEach((defendingType) => {
    allTypes.forEach((attackingType) => {
      const modifier = typeChart[attackingType]?.[defendingType] || 1;
      effectiveness[attackingType] *= modifier;
    });
  });
